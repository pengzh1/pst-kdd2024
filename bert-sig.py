...
def prepare_bert_input():
    ...
    for i in range(4):
        ...
        with open(join(data_dir, f"sig_bib_context_train_{fold}.txt"), "w", encoding="utf-8") as f:
            for line in x_train:
                f.write(line + "\n")
        with open(join(data_dir, f"sig_bib_context_valid_{fold}.txt"), "w", encoding="utf-8") as f:
            for line in x_valid:
                f.write(line + "\n")

        with open(join(data_dir, f"sig_bib_context_train_label_{fold}.txt"), "w", encoding="utf-8") as f:
            for line in y_train:
                f.write(str(line) + "\n")

        with open(join(data_dir, f"sig_bib_context_valid_label_{fold}.txt"), "w", encoding="utf-8") as f:
            for line in y_valid:
                f.write(str(line) + "\n")
...
def train(fold,year=2023, model_name="scibert"):
    ...
    with open(join(data_year_dir, f"sig_bib_context_train_{fold}.txt"), "r", encoding="utf-8") as f:
        for line in f:
            train_texts.append(line.strip())
    with open(join(data_year_dir, f"sig_bib_context_valid_{fold}.txt"), "r", encoding="utf-8") as f:
        for line in f:
            dev_texts.append(line.strip())

    with open(join(data_year_dir, f"sig_bib_context_train_label_{fold}.txt"), "r", encoding="utf-8") as f:
        for line in f:
            train_labels.append(int(line.strip()))
    with open(join(data_year_dir, f"sig_bib_context_valid_label_{fold}.txt"), "r", encoding="utf-8") as f:
        for line in f:
    ...
    MODEL_FILE_NAME = f"pytorch_model_sig_{fold}.bin"
    PATIENCE = 5
...
def gen_kddcup_submission_bert(fold,fkey="train",sfile= "paper_source_trace_train_ans.json"):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("device", device)
    model.load_state_dict(torch.load(join(OUT_DIR, "kddcup", model_name, f"pytorch_model_sig_{fold}.bin")))
    model.to(device)
    model.eval()
    BATCH_SIZE = 32
    ...
    for paper in tqdm(papers):
        ...
        for ii in range(len(predicted_scores)):
            scoreMap[cur_pid][str(idxs[ii])].append(float(sigmoid(predicted_scores[ii])))
    dump_json(scoreMap, join(OUT_DIR, "kddcup", model_name), f"{fkey}_sig_bert_{fold}.json")
